package com.smartonet.project.core.bean;
...
public class Date extends java.util.Date {
     ...
     * 功能：计算两个时间的时间差。
     * @author yongh.yang
     * 2016.07.25 2:34:08 PM
     * @param time 另一个时间。
     * @return long 时间间隔
     */
    public long substract(Date time){
     ...
     * @return
     */
    public int diffValue(Date time){
    	Date day = this.setHourNew(0).setMinuteNew(0).setSecondNew(0);
    	time = time.setHourNew(0).setMinuteNew(0).setSecondNew(0);
    	return (int)(day.getTime()/1000 - time.getTime()/1000)/(24*60*60) ;
    }
     
    /**
     * 功能：当前时间增加毫秒数。
     * @author yongh.yang
     * 2016.07.25 11:26:27 AM
     ...
     * @return String 如：三
     */
    public String dayOfWeekStr(String laguage) {
    	String[] dayNames = { "日", "一", "二", "三", "四", "五", "六" };
    	if("en".equals(laguage)){
    		String[] dayNamesEng = { "Sun.", "Mon.", "Tue.", "Wen.", "Thr.", "Fri.", "Sat." };
    ...
    /**
     * 将日期转换成长中文日期字符串 例如：2009年09月09日 01时01分01秒
     * @return String
     */
    public String toChineseLongDate() {
        DateFormat df = new SimpleDateFormat("yyyy年MM月dd日 HH时mm分ss秒");
        return (null == this) ? null : df.format(this);
    }

    /**
     * 将日期按照自定义的格式进行格式化为字符串。<br/>
     * 例如想将时间格式化为2012-03-05 12:56 ,则只需要传入format为yyyy-MM-dd HH:mm即可。
